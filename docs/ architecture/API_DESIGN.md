...
Source of truth: `GUARDIAN_ARCHITECTURE.md` §8 (Inter-Engine APIs) defines the *internal* module contracts between engines. This document defines the *external* HTTP surface — what a browser, mobile app, or institutional integration actually calls — and maps each route to the internal contract(s) it invokes.
...
1. Every route maps to exactly one entry point into the engine chain — a route that needs to call two unrelated engines directly is a sign the Investigation Engine should be orchestrating instead (`GUARDIAN_ARCHITECTURE.md` Architecture Principle #4).
2. Request/response bodies are validated with Zod schemas that mirror the `AgentInput` / `AgentFinding` / `Finding` shapes in `GUARDIAN_ARCHITECTURE.md` §5 and §7 — the API layer should not invent its own parallel types.
...
**Current State:** `format=user` (web report page) and `format=pdf` (browser print path, demonstrated in the Investigation File 001 prototype) are the only two with any working implementation. The rest are unbuilt — see `GUARDIAN_ARCHITECTURE.md` §4.5.
...
**Target State:** an internal-only route surface, not exposed to end users — the validation-step policy question flagged in `GUARDIAN_ARCHITECTURE.md` §4.6 has to be resolved before this is built, since this route is where that policy gets enforced in code.
