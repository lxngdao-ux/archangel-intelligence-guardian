...
Source of truth: `ARCHITECTURE.md`. This document expands §10 (Roadmap Reconciliation) into concrete milestones and exit criteria. If a milestone here can't be traced to an engine in `ARCHITECTURE.md` §4, it doesn't belong in this roadmap yet — extend the architecture first.
...
This is the current bottleneck (`ARCHITECTURE.md` §10). The Risk Engine is ahead of the Evidence Graph and Explainability Engines it should be reading from. Phase 2 closes that gap before Phase 3 or Phase 5 gets more investment.

**2a — Evidence Graph schema migration**
- Ship `Entity`, `EntityRelationship`, `Observation` (`ARCHITECTURE.md` §7).
- Migrate the Risk Engine to read from `getEvidenceSnapshot()` (`ARCHITECTURE.md` §8) instead of the fraud-pattern scanner directly.
...
- Ship `Finding`, `Hypothesis`, `OutstandingQuestion` (`ARCHITECTURE.md` §7).
...
- Resolve the naming reconciliation from `ARCHITECTURE.md` §5: adopt **relabel-as-roadmap** (recommended) unless a conscious decision is made to rename shipped agents instead.
...
- Exit criteria: all six target agent responsibilities (`ARCHITECTURE.md` §5) are covered by a named, testable agent — whether that's five agents with clear dual responsibilities or six agents 1:1, that decision should be recorded here once made.
...
- Pattern matching against the Knowledge Layer (`ARCHITECTURE.md` §4.7).
...
- Bank / regulator / insurer API integrations (`generateReport()` format targets from `ARCHITECTURE.md` §8 actually implemented, not just typed).
...
- Real-time intervention (Maturity Model Level 4, `ARCHITECTURE.md` §9) — not attempted before Level 3 is real.
- Automated Learning Engine writes without human validation — the validation-step policy question in `ARCHITECTURE.md` §4.6 has to be resolved first.
- Renaming shipped agents purely for pitch-deck naming consistency (`ARCHITECTURE.md` §5) — explicitly rejected, not deferred.

## Milestone tracking

Each milestone above should become a tracked issue referencing this document and the `ARCHITECTURE.md` section it implements. When a milestone ships, update its status here **and** update the corresponding row in `ARCHITECTURE.md` §3 (Platform Capability Matrix) and §4 (engine status) in the same pull request — a roadmap update that doesn't update the architecture doc's status markers has reintroduced the drift this whole document set exists to prevent.
