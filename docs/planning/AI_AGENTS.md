...
Source of truth: `GUARDIAN_ARCHITECTURE.md` §5. This document expands the agent contract, records the reconciliation decision as adopted, and gives a checklist for adding a new agent.

---

## The agent contract (authoritative — do not diverge without updating `GUARDIAN_ARCHITECTURE.md` §5 first)
...
3. **Agents write to the Evidence Graph Engine only** (`recordFindings()`, `GUARDIAN_ARCHITECTURE.md` §8), never directly to Risk or Report. If a new agent's design has it calling the Risk Engine directly, that's a contract violation, not an optimization.
...
**Reconciliation decision: relabel-as-roadmap (adopted).** Per `GUARDIAN_ARCHITECTURE.md` §5, existing agents keep their current names and boundaries. The target set is a roadmap description of coverage, not a renaming mandate:
...
1. Does its output fit the `AgentFinding` contract above without stretching it? If not, the contract may need to change first — propose that change in `GUARDIAN_ARCHITECTURE.md` §5, don't quietly extend it in code.
