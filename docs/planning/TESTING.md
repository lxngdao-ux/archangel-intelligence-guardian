...
Source of truth: `GUARDIAN_ARCHITECTURE.md` Architecture Principle #8 — every investigation is reproducible. Testing exists to enforce that principle, not just to catch bugs.
...
A Guardian output is a chain: Evidence → Observation → Finding → Hypothesis → Risk Score → Report. If any link in that chain can silently change behavior between releases — a prompt tweak that shifts a score, a weight constant that gets rebalanced without anyone noticing — the platform has quietly broken its own Golden Rule (`GUARDIAN_ARCHITECTURE.md` §1) without a single line of code looking wrong in review. Testing strategy here is built around making that kind of silent drift loud instead.

## The canonical fixture

**The redacted Investigation File 001 case (Fynor Trading) is the platform's shared regression fixture.** It's real, it's already fully redacted (name, phone, email, password, and ID number stripped — see the case's own documentation), and its expected output is known and documented across `GUARDIAN_ARCHITECTURE.md` and the Investigation File 001 prototype: 94/100, Pyramid Scheme as the leading supported hypothesis, the specific risk contributor breakdown in `GUARDIAN_ARCHITECTURE.md` §10.
